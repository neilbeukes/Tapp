...
export class TeamComponent implements OnInit {
  ...
  deleteTeamMember() {
    this.service.delete(this.selectedEmployee._id).subscribe(response => {
      this.toastr.warning(
        this.selectedEmployee.FIRSTNAME + " deleted successfully",
        "Teammember"
      );
      this.getTeamMembers();
    });
  ...
  canEdit() {
    if (
      this.auth.getCurrentUserId().toLowerCase() ===
      this.selectedEmployee.USERID.toLowerCase()
    ) {
      return false;
    } else {
      return true;
  ...
  isAdmin() {
    if (
      this.auth.getCurrentUserId().toLowerCase() === "abnb559" ||
      this.auth.getCurrentUserId().toLowerCase() === "abcj423"
    ) {
      return false;
    } else {
      return true;
