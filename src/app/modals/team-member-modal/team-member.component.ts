...
export class TeamMemberComponent {
  ...
  addTeamMember() {
    ...
    this.tmService.add(this.teamMember).subscribe(response => {
      ...
      if (this.title === "Please fill in your profile") {
        localStorage.setItem(
          "username",
          this.teamMember.NAME + " " + this.teamMember.SURNAME
        );
      }
  ...
  updateTeamMember() {
    this.tmService.update(this.teamMember).subscribe(response => {
